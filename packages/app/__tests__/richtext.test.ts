...
describe('Rich text tag-to-style mapping', () => {
   ...
   */
  function computeEffectiveStyle(
    ancestorStyle: TextStyle,
    tag: string,
  ): TextStyle {
...
describe('parseRichText unit tests', () => {
  ...
  it('should treat malformed/unrecognized tags as plain text', () => {
    ...
    const hasHello = nodes.some(
      (n) => n.type === 'text' && n.text.includes('Hello'),
    );
    const hasWorld = nodes.some(
      (n) => n.type === 'text' && n.text.includes('world'),
    );
